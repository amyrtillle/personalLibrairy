        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h2 class="text-lg font-medium text-gray-900">Nouvel examplaire de {{ $livre_titre }}</h2>
                    @if(session('status'))
                    ...
                    <form action="{{ route('livre.exemplaires.store', $livre_id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <a class="inline-flex items-right px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest"
                href="{{ route('livre.exemplaires.index', $livre_id) }}">
                Retour
            </a>
        </div>
    </div>
